export const docsPages = [
  ...
  },
  {
    "slug": "environments",
    "title": "Environments",
    "description": "Separate development, staging, and production configurations for safe deployments.",
    "sections": [
      {
        "heading": "Environment Overview",
        "body": "VectorStack supports isolated environments so you can develop and test without affecting live data. Each environment has its own connection string, indexes, and API keys. Manage keys per environment in the <a href=\"/docs/api-keys\">API keys guide</a>."
      },
      {
        "heading": "Creating Environments",
        "body": "Navigate to Settings > Environments in the VectorStack console. Click 'Add Environment' and select a region. Each environment receives a unique base URL that you pass to the SDK during initialization. Review SDK configuration in the <a href=\"/sdk/javascript\">JavaScript SDK</a> documentation.",
        "code": "const client = new VectorStack({\n  apiKey: process.env.VECTORSTACK_API_KEY,\n  environment: 'staging'\n});"
      },
      {
        "heading": "Environment Variables",
        "body": "Store environment-specific credentials in .env files or secret managers. Never commit API keys to version control. Read our <a href=\"/docs/security\">security guidelines</a> for recommended secret storage patterns."
      },
      {
        "heading": "Promoting Between Environments",
        "body": "When you are ready to move index configurations from staging to production, use the promote command. This copies index schemas and settings without duplicating vector data. Verify promotion results inside <a href=\"/docs/observability\">observability dashboards</a>.",
        "code": "vectorstack env promote --from staging --to production --index support-kb"
      },
      {
        "heading": "Verification Guidelines",
        "body": "Ensure that connection handshakes are executed within the sandboxed environment to isolate index operations. Run test queries to verify metadata filters align correctly with your access scopes."
    ...
    "nextSteps": [
      {
        "text": "Security Guide",
        "description": "Protect credentials across all environments.",
        "href": "/docs/security"
      },
      {
        "text": "Teams & Permissions",
        "description": "Restrict environment access by team role.",
        "href": "/docs/teams-and-permissions"
      }
    ]
  },
  {
    ...
    "description": "Configure role-based access control for your VectorStack organization members.",
    "sections": [
      {
        "heading": "Role Types",
        "body": "VectorStack defines three built-in roles: Owner, Administrator, and Read-Only. Owners can manage billing and delete organizations. Administrators can create indexes and rotate keys. Read-Only members can query indexes but cannot modify configurations. Check key scopes in our <a href=\"/docs/api-keys\">API keys guide</a>."
      },
      {
        "heading": "Inviting Team Members",
        "body": "Navigate to Settings > Team in the console. Enter the email address and select a role. The invitee receives an email with a magic link to join your organization. SSO integration is available on Enterprise plans — see <a href=\"/pricing\">pricing</a> for details."
      },
      {
        "heading": "Custom Roles",
        "body": "Enterprise customers can create custom roles with granular permissions. Permissions are scoped to specific environments and indexes. For example, you can allow a team member to search a production index but not delete documents from it.",
        "code": "{\n  \"role\": \"search-analyst\",\n  \"permissions\": [\n    \"indexes:search\",\n    \"indexes:read\",\n    \"documents:read\"\n  ],\n  \"scope\": {\n    \"environments\": [\"production\"],\n    \"indexes\": [\"support-kb\", \"docs-index\"]\n  }\n}"
      },
      {
        "heading": "Audit Trail",
        "body": "All permission changes are logged in the organization audit trail. Review changes via the <a href=\"/api/audit-logs\">audit logs API</a> or in the console under Settings > Audit Log."
      },
      {
        "heading": "Verification Guidelines",
        "body": "Ensure that connection handshakes are executed within the sandboxed environment to isolate index operations. Run test queries to verify metadata filters align correctly with your access scopes."
    ...
    "nextSteps": [
      {
        "text": "Security Standards",
        "description": "Review encryption and compliance configurations.",
        "href": "/docs/security"
      },
      {
        "text": "Audit Logs",
        "description": "Track all organizational changes programmatically.",
        "href": "/api/audit-logs"
      }
    ]
  },
  {
    "slug": "observability",
    "title": "Observability",
    "description": "Monitor search latency, ingestion throughput, and index health with built-in dashboards.",
    "sections": [
      {
        "heading": "Dashboard Overview",
        "body": "The Observability dashboard provides real-time metrics for all your indexes. Track search latency (p50, p95, p99), ingestion throughput, error rates, and storage utilization. Access it from the console sidebar or query metrics via the <a href=\"/api/usage\">usage API</a>."
      },
      {
        "heading": "Search Latency Monitoring",
        "body": "Monitor query performance with latency percentiles. Set alerts when p95 latency exceeds your SLA threshold. Latency spikes often correlate with index size growth or complex metadata filters. Review filter optimization in the <a href=\"/docs/metadata-filtering\">metadata filtering guide</a>."
      },
      {
        "heading": "Ingestion Metrics",
        "body": "Track documents processed per minute, failed ingestions, and average chunk sizes. Configure <a href=\"/docs/webhooks\">webhook alerts</a> to notify your team when ingestion failures spike above a threshold."
      },
      {
        "heading": "Custom Alerts",
        "body": "Create custom alert rules based on any metric. Alerts can be delivered via email, Slack, or webhook endpoints. Configure alert channels in Settings > Alerts.",
    ...
    "relatedLinks": [
      ...
      { "text": "Webhooks", "href": "/docs/webhooks" },
      { "text": "Rate Limits", "href": "/docs/rate-limits" },
      { "text": "Errors", "href": "/docs/errors" }
    ],
    "nextSteps": [
      {
        "text": "Usage API Reference",
        "description": "Query metrics programmatically via REST endpoints.",
        "href": "/api/usage"
      },
      {
        "text": "Webhook Alerts",
  ...
  },
  {
    "slug": "evaluation",
    "title": "Evaluation",
    "description": "Measure and validate retrieval quality, context recall, and RAG pipeline accuracy.",
    "sections": [
      {
        "heading": "Why Evaluate",
        "body": "RAG systems can silently degrade as your data grows or changes. Regular evaluation ensures your search pipeline continues to return accurate, relevant results. Track quality trends in your <a href=\"/docs/observability\">observability dashboard</a>."
      },
      {
        "heading": "Key Metrics",
        "body": "VectorStack supports four core evaluation metrics: Precision@K (how many of the top K results are relevant), Recall@K (how many relevant documents appear in the top K), Mean Reciprocal Rank (position of the first relevant result), and Context Relevance (semantic similarity between retrieved chunks and the expected answer)."
      },
      {
        "heading": "Running Evaluations",
        "body": "Create an evaluation dataset with query-answer pairs. Run the evaluation command to score your index against the golden dataset. Results are saved to the observability dashboard.",
        "code": "const evalResults = await client.evaluate({\n  indexId: 'support-kb',\n  dataset: [\n    { query: 'How to rotate keys?', expectedDocId: 'doc-api-keys' },\n    { query: 'What is chunking?', expectedDocId: 'doc-chunking' }\n  ],\n  metrics: ['precision@5', 'recall@5', 'mrr']\n});\nconsole.log(evalResults.summary);"
      },
      {
        "heading": "Continuous Evaluation",
        "body": "Schedule recurring evaluations using cron jobs or CI/CD pipelines. Compare results across releases to catch regressions. Read about automated testing in the <a href=\"/docs/rag-pipelines\">RAG pipelines guide</a>."
      },
      {
        "heading": "Verification Guidelines",
        "body": "Ensure that connection handshakes are executed within the sandboxed environment to isolate index operations. Run test queries to verify metadata filters align correctly with your access scopes."
    ...
    "nextSteps": [
      {
        "text": "RAG Pipelines",
        "description": "Build and optimize retrieval-augmented generation flows.",
        "href": "/docs/rag-pipelines"
      },
      {
        "text": "Reranking Setup",
        "description": "Improve retrieval accuracy with cross-encoder models.",
        "href": "/docs/reranking"
      }
    ]
  },
  {
    ...
    "description": "Understand how VectorStack stores, archives, and deletes your vector data.",
    "sections": [
      {
        "heading": "Retention Policies",
        "body": "By default, all data stored in VectorStack indexes is retained indefinitely until explicitly deleted. Free tier indexes that remain inactive for 30 consecutive days are automatically archived. Review plan limits in our <a href=\"/pricing\">pricing guide</a>."
      },
      {
        "heading": "Automatic Archiving",
        "body": "Archived indexes are moved to cold storage. Search queries against archived indexes return a 410 Gone error. To restore an archived index, navigate to Settings > Indexes and click 'Restore'. Restoration takes 2-5 minutes depending on index size."
      },
      {
        "heading": "Manual Deletion",
        "body": "You can delete individual documents or entire indexes. Deletion is permanent and cannot be undone. To delete a document, use the documents API. To delete an index, use the indexes API. Review endpoints in the <a href=\"/api/documents\">documents API reference</a>.",
        "code": "// Delete a single document\nawait client.documents.delete({ indexId: 'kb', documentId: 'doc-123' });\n\n// Delete an entire index\nawait client.indexes.delete({ indexId: 'kb' });"
      },
      {
        "heading": "Compliance & GDPR",
        "body": "VectorStack supports GDPR right-to-erasure requests. Submit deletion requests through Settings > Compliance or via the API. All associated vectors and metadata are purged within 48 hours. Audit deletion events in the <a href=\"/api/audit-logs\">audit logs</a>."
      },
      {
        "heading": "Verification Guidelines",
        "body": "Ensure that connection handshakes are executed within the sandboxed environment to isolate index operations. Run test queries to verify metadata filters align correctly with your access scopes."
    ...
    "relatedLinks": [
      ...
      { "text": "Audit Logs", "href": "/api/audit-logs" }
    ],
    "nextSteps": [
      {
        "text": "Vector Indexes",
        "description": "Learn about index lifecycle and management.",
        "href": "/docs/vector-indexes"
      },
      {
        "text": "Security Guide",
        "description": "Review encryption and compliance standards.",
        "href": "/docs/security"
      }
    ]
  }
